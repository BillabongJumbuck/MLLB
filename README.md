...
```shell
sudo stress-ng --cpu 8 --io 8 --fork 4 --vm 4 --vm-bytes 256M --timeout 120s
sudo stress-ng --cpu 0 --cpu-method all --timeout 180s
sudo stress-ng --cpu 4 --io 4 --vm 2 --vm-bytes 512M --timeout 250s
sudo stress-ng --fork 8 --switch 8 --timeout 60s
```
...
1. 开启测试
   ...
   ​	作者定义了一个名为 jc_sched 的系统调用，接受一个整型参数 start. 当start非零时, 系统在每次触发负载均衡决策时, 会将CFS和ML的决策结果和耗时同时打印到日志中, 从而对比ML决策的准确率和时间花费.当start为0时, 停止打印日志.
...
​	通过`python3 dump_weights.py model_0001`可以输出当前训练得到的MLP的各层的权重和偏置.为了将这些新的参数修改到内核中使用, 需要重新编译安装内核.

1. 打印MLP各层参数

   在`MLLB/training`中运行`python3 dump_weights.py model_0001`打印出训练出的MLP的各层的权重和偏置.

 1. 在源代码中更新参数
    在`linux-4.15-lb/kernel/sched/jc_mlp.h`中更新各层的权重和偏置.

 2. 在`linux-4.15-lb`目录中,重新运行:
...
3. 卸载旧的4.15-lb内核
   ...
   # 卸载旧内核
   sudo rm /boot/*-4.15.0+
   sudo rm /boot/initrd.img-4.15.0+
   sudo rm /boot/vmlinuz-4.15.0+.old
   ```
